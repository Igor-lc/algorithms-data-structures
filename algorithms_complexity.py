...
[1, 3, 5, 7, 9, 11]
[11, 1, 3, 7, 9, 5]
В обоих случаях нам нужно найти число 11. In both cases, we need to find the number 11. Очевидно, что в 1 случае цикл пройдет по всем элементам, так как искомое значение находится в конце. It is evident that in the first scenario, the loop will traverse through all the elements since the desired value is located at the end. То есть будет произведено n итераций, где n – это длина списка. Thus, n iterations will be performed, where n is the length of the list. Во втором случае будет выполнена всего 1 итерация, так как искомое значение стоит в самом начале. In the second case, only 1 iteration will be executed, as the sought value is at the very beginning. Если бы мы опирались на фактические данные, то в 1 случае сложность алгоритма была бы О(n), а во 2 О(1), то есть разница в скорости работы алгоритма между 2 наборами данных — существенна. If we relied on actual data, the complexity of the algorithm in the first case would be O(n), while in the second case, it would be O(1), signifying a significant difference in the algorithm's speed between the two data sets. Но на практике при анализе мы исследуем не фактические данные, а оцениваем сам алгоритм, поэтому нужно всегда исходить из наиболее не благоприятного случая. То есть есть смысл рассматривать только 1 вариант. However, in practice, during analysis, we explore not actual data but rather assess the algorithm itself. Hence, we must always consider the worst-case scenario, meaning we should focus on the first scenario. А значит сложность алгоритма линейного поиска О(n). Consequently, the complexity of the linear search algorithm is O(n). При этом мы учитываем, что при определенных наборах данных алгоритм отработает гораздо быстрее. While doing so, we take into account that the algorithm will perform much faster under certain data sets. Поэтому есть смысл утверждать, что алгоритм работает или плохо как О(n), или, скорее всего — лучше.  Therefore, it makes sense to assert that the algorithm performs poorly in terms of O(n) or, most likely, better. То есть, наш алгоритм асимтотически не хуже, чем О(n). In other words, our algorithm is asymptotically no worse than O(n). Существует и более математический способ определения сложности этого алгоритма. There is a more mathematical way to determine the complexity of this algorithm. Будем исходить из того, что все элементы в списке расположены в случайном порядке и искомое значение может с равной вероятностью находится в любом месте. То есть в вероятностью 1/n. Let's assume that all elements in the list are randomly ordered, and the sought value could be in any position with an equal probability of 1/n. При этом если элемент будет на 1 итерации, то цикл выполнится (1 х 1/n) раз. If the element happens to be at the 1st iteration, the loop will execute (1 x 1/n) times. А если он будет стоять в конце или искомого элемента вообще не будет, то цикл, то цикл выполнится (n x 1/n) раз. If it's at the end or missing entirely, the loop will execute (n x 1/n) times. В итоге, мы получаем следующее среднее время поиска для всех элементов: (1+2+ … +n) / n = (n+1) / 2. То есть успешный поиск методом перебора в среднем занимает O((n+1) / 2) = O(n) – с учетом всех упрощений. Ultimately, we obtain the following average search time for all elements: (1+2+ … +n) / n = (n+1) / 2. This implies that successful brute-force search on average takes O((n+1) / 2) = O(n) – considering all simplifications. Вообще на практике такие расчеты обычно не делаются. In practice, such calculations are generally not performed.  Программисты исходят из того, что одиночный цикл имеет сложность O(n). В большинстве случаев этого достаточно. Но это если циклы стоят рядом. Programmers typically assume that a single loop has a complexity of O(n), which is sufficient for most cases. При этом если в алгоритме друг за другом стоят 2 цикла, то сложность так же считается как O(n). Если же циклы вложены друг в друга, то сложность алгоритма с учетом всех упрощений увеличивается квадратично. If loops are nested within each other, the algorithm's complexity, with all simplifications considered, increases quadratically. То есть вложенные циклы имеют квадратичную сложность — О(n2). Thus, nested loops have quadratic complexity — O(n2).
